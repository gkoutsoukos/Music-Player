# Particle Egg Music Player v9 — Google Drive Picker
...
Added the first working Google Drive selection flow.

How to test:

1. Make sure `js/config.js` contains your real:
   - Google Client ID
   - Google API key
   - scope: `https://www.googleapis.com/auth/drive.file`

2. In Google Cloud Console, make sure:
   - Drive API is enabled.
   - Google Picker API is enabled if it appears as a separate API in your console.
   - Your OAuth Client is a Web application.
   - Authorized JavaScript origins include your GitHub Pages URL, for example:
...
5. Click/tap the orb to open the playlist.

6. In the empty playlist, choose:
   - Load local audio files
   - Load from Google Drive

7. Sign in with Google and select audio files.

Notes:

- The main UI remains clean. There are no visible buttons below the timeline.
- Google Drive loading is available from the empty playlist screen.
- Selected Drive files are downloaded into the browser as blobs and then handed to the existing player.
- This version uses `drive.file` scope, keeping access limited to user-selected files.
